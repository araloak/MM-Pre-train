...
from ..data.feature_sequence_mlm_dataset  import FeatureSeqClsMlmDataset
from ..data.feature_dataset import FeatureDataset
from ..data.utils import text_bin_file
import numpy as np
@register_task('img-seq-cls-mlm')
class ImgSeqClsMlmTask(FairseqTask):
    @staticmethod
    def add_args(parser):
        parser.add_argument('--data-dir', default='output',
                            help='data directory')
        parser.add_argument('--img-dim', type=int, default=1000,
                            help='m')
        parser.add_argument('--img-seq-len', type=int, default=6,
                            help='m')
        parser.add_argument('--max-src-sent', type=int, default=5,
                            help='max source sentence num')
        parser.add_argument('--num-classes', type=int, metavar='N', default=7,
    ...
    def setup_task(cls, args, **kwargs):
        ...
        return ImgSeqClsMlmTask(args, vocab_dict)
    ...
    def load_dataset(self, split, **kwargs):
        ...
        self.datasets[split] = FeatureSeqClsMlmDataset(
