...
from ..data.all_data_dataset import AllTaskDataset

@register_task('all-task')
class AllTask(FairseqTask):
    @staticmethod
    def add_args(parser):
        parser.add_argument('--data-dir', default='output',
                            help='data directory')
        parser.add_argument('--analyse-dir', default='',
                            help='data directory')
    ...
    def setup_task(cls, args, **kwargs):
        ...
        return AllTask(args, vocab_dict)
    ...
    def load_dataset(self, split, **kwargs):
        ...
        #cls
        cls_labels = open(label_file(self.args.data_dir+"/"+self.args.analyse_dir, "cls_only." + split), encoding="utf8").readlines()
        cls_labels = [int(each.strip()) for each in cls_labels]
        
        cls_only_features_dataset = np.memmap(feature_file(self.args.data_dir+"/"+self.args.analyse_dir, "cls_only." + split), dtype='float32', mode='r',shape=(len(cls_labels), 1000))

        self.datasets[split] = AllTaskDataset(data_dir=self.args.data_dir,
