...
CLS_TOKEN = 2
@register_model("img-cls-mlm-bart")
class ImageBARTClsMlmModel(TransformerModel):
    ...
    def forward(self,
        ...
        encoder_out = self.encoder(src_tokens,
                                   ...
                                   **kwargs)
        encoder_token_padding_mask = src_tokens.eq(self.mask_index) ##todo dynamicly get “<mask>” token index


        img_representation = encoder_out.encoder_out # todo : extract CLS vevtor from encoder_out
        x1 = self.img_classification_layer(
            img_representation[1:max_img_num+1,:,:] 
        )
        x2 = self.mlm_classification_layer(
            img_representation[max_img_num+1:,:,:].transpose(0,1)[encoder_token_padding_mask] 
        ) 
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return ImageBARTClsEncoder(args, src_dict, embed_tokens)
...
class ImageBARTClsEncoder(TransformerEncoder):
    """
...
@register_model_architecture('img-cls-mlm-bart', 'baseline-img-cls-mlm-bart')
...
@register_model_architecture("img-cls-mlm-bart", "baseline-img-cls-mlm-bart-base")
...
@register_model_architecture("img-cls-mlm-bart", "baseline-img-cls-mlm-bart-trimed")
def bart_base_architecture(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 768)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 4 * 768)
